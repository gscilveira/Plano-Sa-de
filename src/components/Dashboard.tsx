...
export default function Dashboard({ 
  ...
  return (
    <div className={`space-y-8 ${isLightMode ? 'text-black' : 'text-white'}`}>
        ...
        {activeTab === 'resumo' ? (
          ...
          >
            <div className="flex flex-col gap-2 px-1">
              <h2 className={`text-3xl xs:text-4xl sm:text-5xl md:text-7xl font-bold tracking-tighter uppercase italic leading-none ${isLightMode ? 'text-black' : 'text-white'}`}>
                {plan.biotype}
              </h2>
              <p className="text-wellness-green text-[10px] sm:text-xs tracking-[0.2em] sm:tracking-[0.4em] font-black uppercase">Bio-Algoritmo Detectado</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
               <div className="glass-card p-5 sm:p-8 rounded-[24px] sm:rounded-[32px] border-wellness-green/10 flex flex-col justify-center">
                  <div className={`text-[10px] uppercase font-black tracking-widest mb-1 ${isLightMode ? 'text-black/30' : 'text-white/30'}`}>IMC</div>
                  <div className="text-3xl sm:text-4xl font-mono text-wellness-green">{Math.floor(plan.imc)}</div>
               </div>
               <div className="glass-card p-5 sm:p-8 rounded-[24px] sm:rounded-[32px] border-wellness-green/10 flex flex-col justify-center">
                  <div className={`text-[10px] uppercase font-black tracking-widest mb-1 ${isLightMode ? 'text-black/30' : 'text-white/30'}`}>Meta Kcal</div>
                  <div className="text-3xl sm:text-4xl font-mono text-wellness-green">{Math.floor(plan.dailyCalories)}</div>
               </div>
               <div className={`glass-card p-5 sm:p-8 rounded-[24px] sm:rounded-[32px] flex flex-col justify-center ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
                  <div className={`text-[10px] uppercase font-black tracking-widest mb-1 ${isLightMode ? 'text-black/30' : 'text-white/30'}`}>Gasto (TDEE)</div>
                  <div className="text-3xl sm:text-4xl font-mono">{Math.floor(plan.tdee)}</div>
               </div>
               <div className={`glass-card p-5 sm:p-8 rounded-[24px] sm:rounded-[32px] flex flex-col justify-center ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
                  <div className={`text-[10px] uppercase font-black tracking-widest mb-1 ${isLightMode ? 'text-black/30' : 'text-white/30'}`}>Base (TMB)</div>
                  <div className="text-3xl sm:text-4xl font-mono">{Math.floor(plan.tmb)}</div>
               </div>
            </div>

            <div className="glass-card p-6 sm:p-10 rounded-[28px] sm:rounded-[40px] border-wellness-green/20 relative overflow-hidden">
               <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6 sm:gap-8">
                  <div className="space-y-2 text-center md:text-left">
                     <div className="text-[10px] text-wellness-green font-black uppercase tracking-[0.2em] sm:tracking-[0.3em]">Meta de Consistência</div>
                     <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold italic tracking-tighter uppercase">Protocolo de Treino</h3>
                     <p className={`text-xs sm:text-sm font-medium ${isLightMode ? 'text-black/60' : 'text-white/40'}`}>Sua meta é de {trainingDaysGoal} sessões semanais para otimização máxima.</p>
                  </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
               {plan.macros.map((m, i) => (
                 <div key={i} className={`glass-card p-5 sm:p-8 rounded-[24px] sm:rounded-[32px] ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
                    ...
                    <div className="text-3xl sm:text-4xl font-mono">{Math.floor(m.value)}<small className={`text-xs ml-2 ${isLightMode ? 'text-black/30' : 'text-white/30'}`}>{m.unit}</small></div>
                 </div>
               ))}
            </div>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-4 px-4">
               ...
               <h2 className="text-2xl xs:text-3xl sm:text-4xl font-bold tracking-tighter uppercase italic">Protocolo Alimentar</h2>
            </div>
            
            <div className="grid grid-cols-1 gap-4">
               {plan.meals.map((meal, i) => (
                 <div key={i} className={`glass-card p-5 sm:p-10 rounded-[28px] sm:rounded-[40px] flex flex-col md:flex-row md:items-center gap-4 sm:gap-10 hover:border-wellness-green/20 transition-all group ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
                    <div className={`flex flex-col min-w-[120px] md:border-r ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
                       ...
                       <span className={`text-xl sm:text-2xl font-bold tracking-tighter uppercase group-hover:text-wellness-green transition-colors ${isLightMode ? 'text-black' : 'text-white'}`}>{meal.name}</span>
                    </div>
                    <div className="flex-1">
                       <p className={`text-sm sm:text-base md:text-lg leading-relaxed font-medium ${isLightMode ? 'text-black/60' : 'text-white/50'}`}>
                          {meal.foods.join(" / ")}
                       </p>
                    </div>
                    <div className="text-right">
                       <span className={`text-2xl sm:text-3xl font-mono font-bold block ${isLightMode ? 'text-black' : 'text-white'}`}>{meal.calories}</span>
          ...
          >
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 px-4">
               <div>
                  <h2 className={`text-2xl xs:text-3xl sm:text-4xl md:text-6xl font-bold tracking-tighter italic uppercase underline decoration-wellness-green decoration-4 ${isLightMode ? 'text-black' : 'text-white'}`}>Performance Semanal</h2>
                  <p className={`${isLightMode ? 'text-black/30' : 'text-white/20'} text-[9px] sm:text-[10px] tracking-[0.15em] sm:tracking-[0.5em] font-black uppercase mt-4`}>META: {trainingDaysGoal} SESSÕES ATIVAS / {(trainingProgress).toFixed(0)}% DE ADERÊNCIA</p>
               </div>
               ...
               {weekDays.map((day, i) => (
                 <button
                   key={i}
                    ...
                    className={`flex flex-col items-center gap-1.5 sm:gap-3 p-1.5 xs:p-2 sm:p-4 rounded-[16px] sm:rounded-[24px] border transition-all duration-300 ${
                      selectedDay === i 
                  ...
                  >
                    <span className={`text-[8px] xs:text-[9px] sm:text-[10px] font-black uppercase tracking-wider sm:tracking-widest ${selectedDay === i ? 'text-black' : isLightMode ? 'text-black/30' : 'text-white/30'}`}>{day}</span>
                    <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center border transition-all ${
                      selectedDay === i ? 'bg-black border-black text-wellness-green' : isLightMode ? 'border-black/10 bg-black/5' : 'border-white/5 bg-white/5'
                    }`}>
                       {checkedDays.includes(i) ? <CheckCircle2 className="w-3 h-3 sm:w-4 sm:h-4" /> : <div className={`w-1 h-1 sm:w-1.5 sm:h-1.5 rounded-full ${isLightMode ? 'bg-black/10' : 'bg-white/10'}`} />}
                    </div>
                  </button>
               ))}
            </div>
            ...
            {/* Workout Details */}
            <div className={`glass-card p-5 sm:p-10 rounded-[28px] sm:rounded-[48px] ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
                   <div>
                      <h3 className={`text-2xl sm:text-3xl font-bold tracking-tight italic uppercase ${isLightMode ? 'text-black' : 'text-white'}`}>{plan.weeklyWorkout[selectedDay]?.day || weekDays[selectedDay]}</h3>
                   ...
                   <button 
                     ...
                     className={`w-full md:w-auto px-8 py-3.5 rounded-full text-[10px] font-black tracking-widest border transition-all text-center ${
                ...
                <div className="space-y-4">
                   {plan.weeklyWorkout[selectedDay]?.exercises.map((ex, i) => (
                     <div key={i} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 xs:p-5 sm:p-6 rounded-[20px] sm:rounded-[24px] border hover:border-wellness-green/30 group transition-all ${isLightMode ? 'bg-black/5 border-black/5' : 'bg-white/5 border-white/5'}`}>
                        <div className="flex items-center gap-6">
                           ...
                           <h4 className={`font-bold text-sm sm:text-base md:text-lg ${isLightMode ? 'text-black' : 'text-white'}`}>{ex.name}</h4>
                        </div>
          ...
          >
             <div className="relative w-64 h-64 sm:w-80 sm:h-80">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                   <Droplets className={`w-8 h-8 sm:w-10 sm:h-10 mb-2 ${isLightMode ? 'text-wellness-green' : 'text-white'}`} />
                   <div className={`text-5xl sm:text-7xl font-mono font-bold ${isLightMode ? 'text-black' : 'text-white'}`}>{waterDrunk}</div>
             ...
             <div className="flex flex-col items-center gap-6">
                <div className="flex flex-wrap justify-center gap-2 sm:gap-3 max-w-xs sm:max-w-md">
                   {Array.from({ length: Math.ceil(plan.waterTarget / 250) }).map((_, idx) => {
                      ...
                      return (
                        <motion.div 
                          ...
                          className={`w-8 h-10 sm:w-10 sm:h-12 rounded-lg sm:rounded-xl flex items-center justify-center border transition-all duration-500 ${
                            isFilled 
                        ...
                        >
                           <Droplets className={`w-4 h-4 sm:w-5 sm:h-5 ${isFilled ? 'fill-current' : ''}`} />
                        </motion.div>
                      );
                   })}
                </div>
             ...
             <div className="flex gap-3 sm:gap-4 items-center">
                <button 
                  ...
                  className="w-20 h-20 sm:w-24 sm:h-24 rounded-[24px] sm:rounded-[32px] glass-card flex flex-col items-center justify-center hover:bg-white/10 transition-all border-white/5 shrink-0"
                >
                  <Minus className={`w-5 h-5 sm:w-6 sm:h-6 mb-1 ${isLightMode ? 'text-black/30' : 'text-white/50'}`} />
                  <span className={`text-[9px] sm:text-[10px] font-black uppercase ${isLightMode ? 'text-black/20' : 'text-white/30'}`}>-500ml</span>
                </button>
                <button 
                   onClick={() => addWater(250)}
                   className="w-24 h-24 sm:w-32 sm:h-32 rounded-[36px] sm:rounded-[48px] bg-wellness-green text-black flex flex-col items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-[0_10px_30px_rgba(198,255,0,0.3)] sm:shadow-[0_20px_50px_rgba(198,255,0,0.4)] shrink-0"
                >
                   <Plus className="w-8 h-8 sm:w-10 sm:h-10 mb-1" />
                   <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest">BEBER</span>
                </button>
                <button 
                  onClick={() => addWater(500)}
                  className="w-20 h-20 sm:w-24 sm:h-24 rounded-[24px] sm:rounded-[32px] glass-card flex flex-col items-center justify-center hover:bg-white/10 transition-all border-white/5 shrink-0"
                >
                   <Plus className={`w-5 h-5 sm:w-6 sm:h-6 mb-1 ${isLightMode ? 'text-black/30' : 'text-white/50'}`} />
                   <span className={`text-[9px] sm:text-[10px] font-black uppercase ${isLightMode ? 'text-black/20' : 'text-white/30'}`}>+500ml</span>
                </button>
             </div>
          </motion.div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8">
               <div className={`glass-card p-5 sm:p-10 rounded-[28px] sm:rounded-[48px] space-y-6 sm:space-y-8 ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
               ...
               <div className={`glass-card p-5 sm:p-10 rounded-[28px] sm:rounded-[48px] flex flex-col justify-between ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
            ...
            <div className={`glass-card p-5 sm:p-10 rounded-[28px] sm:rounded-[48px] ${isLightMode ? 'border-black/5' : 'border-white/5'}`}>
               ...
               <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                  ...
                  <button 
                    onClick={() => onLogout()}
                    className={`w-full md:w-auto px-6 py-3.5 rounded-full border font-black uppercase tracking-widest transition-all text-[10px] flex items-center justify-center gap-2 ${
                      isLightMode 
           ...
           <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-4 px-4">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-wellness-green/10 flex items-center justify-center text-wellness-green shrink-0">
                   <Calendar className="w-5 h-5 sm:w-6 sm:h-6" />
                </div>
                <h2 className={`text-xl sm:text-2xl md:text-3xl font-bold tracking-tighter uppercase italic ${isLightMode ? 'text-black' : 'text-white'}`}>Protocolos Arquivados</h2>
              </div>
              <span className={`text-[10px] font-black uppercase tracking-widest self-start xs:self-auto ${isLightMode ? 'text-black/20' : 'text-white/20'}`}>{history.length} SALVOS</span>
           </div>

           {history.length === 0 ? (
             <div className={`glass-card p-10 sm:p-20 rounded-[28px] sm:rounded-[48px] border-dashed flex flex-col items-center justify-center text-center opacity-50 ${isLightMode ? 'border-black/10' : 'border-white/5'}`}>
      ...
      <nav className={`w-full border-t mt-12 px-2 sm:px-4 py-4 sm:py-6 flex justify-center transition-all duration-300 ${isLightMode ? 'bg-white border-black/10 rounded-[20px] sm:rounded-[32px]' : 'bg-wellness-charcoal/30 border-white/5 rounded-[20px] sm:rounded-[32px]'}`}>
        <div className="flex justify-around items-center w-full max-w-lg gap-1 sm:gap-2">
          {tabs.map(tab => (
            <button
              key={tab.id}
              onClick={() => {
              ...
              className={`flex flex-col items-center gap-1 px-2 xs:px-3 sm:px-5 py-2 sm:py-2.5 rounded-[16px] sm:rounded-[20px] transition-all duration-300 ${
                activeTab === tab.id 
                ? 'bg-wellness-green text-black scale-[1.02] shadow-lg shadow-wellness-green/20 font-black' 
            ...
            >
              ...
              <span className="text-[8px] xs:text-[9px] sm:text-[10px] font-black tracking-tight xs:tracking-widest">{tab.label}</span>
            </button>
          ))}
        </div>
      </nav>
